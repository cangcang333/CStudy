/*************************************************************************
	> File Name: event_multilookup.c
	> Author: 
	> Mail: 
	> Created Time: Sat Jun 23 16:46:00 2018
 ************************************************************************/
...
/* Take a list of domain names from the command line and resolve them in parallel. */
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; ++i)
    {
        ...
        * each address: one for TCP and one for UDP. That's not what we want. */
        ...
        ++n_pending_requests;
        req = evdns_getaddrinfo(dnsbase, argv[i], NULL /* no service name given */, &hints, callback, user_data);
        if (req == NULL)
            printf("    [request for %s returned immediately]\n", argv[i]);
        /* No need to free user_data or decrement n_pending_requests; 
